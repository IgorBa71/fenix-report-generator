...
- Реплики клиента и данные, подставляемые автоматически (в кавычках «...»),
  выделены жирным цветом #1A7572 — чтобы легко отличать "что сказать" от
  "что процитировать"/подставленных данных клиента.
"""
import re
...
DATA_INSERT_COLOR = "#1A7572"


def _inline_format(text):
    """«...» -> цвет/насыщенность, которыми выделены данные клиента,
    автоматически подставляемые в текст Скрипта (Вызовы, КСЭ, цитаты из
    Раздела 9 и т.д.) — жирным, цветом #1A7572, чтобы такие вставки были
    визуально легко отличимы от остального текста."""
    text = _escape(text)
    text = re.sub(r'«([^»]*)»', rf'«<b><font color="{DATA_INSERT_COLOR}">\1</font></b>»', text)
    return text
