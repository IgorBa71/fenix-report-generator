...
- Реплики клиента и данные, подставляемые автоматически (в кавычках «...»),
  выделены шрифтом Roboto-Black цветом #D5530B — чтобы легко отличать "что
  сказать" от "что процитировать"/подставленных данных клиента.
"""
import re
...
def _inline_format(text):
    """«...» -> цвет/насыщенность, которыми выделены данные клиента,
    автоматически подставляемые в текст Скрипта (Вызовы, КСЭ, цитаты из
    Раздела 9 и т.д.) — шрифтом Roboto-Black, цветом ORANGE (#D5530B),
    чтобы такие вставки были визуально легко отличимы от остального текста."""
    text = _escape(text)
    text = re.sub(r'«([^»]*)»', r'«<font face="Roboto-Black" color="#D5530B">\1</font>»', text)
    return text
