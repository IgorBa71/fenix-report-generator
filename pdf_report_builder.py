...
REPORT_COUNTER_START = 354  # последний уже выданный номер до переноса на постоянный диск — следующий будет 355
...
def get_next_report_number():
    ...
    ВАЖНО: это по-прежнему не защищает от гонки условий (race condition) при
    двух одновременных запросах — для этого нужен отдельный атомарный
    счётчик на стороне CRM/БД. Обсуждается отдельно как отложенная задача."""
