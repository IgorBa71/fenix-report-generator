...
REPORT_COUNTER_FILE = BASE / "data" / "report_counter.txt"
REPORT_COUNTER_START = 350  # номер самого первого отчёта


def get_next_report_number():
    """Возвращает следующий по порядку номер отчёта в формате 'ХХХХХХ' (6 цифр
    с ведущими нулями) и сохраняет счётчик в файл на диске, чтобы при
    следующем запуске генератор продолжил нумерацию, а не начинал заново.

    Первый вызов (когда файла-счётчика ещё нет) возвращает REPORT_COUNTER_START.
    Каждый следующий вызов — предыдущее значение + 1.

    В реальном пайплайне (Make.com/CRM) эта же логика должна жить на стороне
    системы, которая фактически создаёт отчёты один за другим — здесь она
    реализована как локальный файл-счётчик просто потому, что скрипт сейчас
    запускается изолированно, без внешней БД."""
    REPORT_COUNTER_FILE.parent.mkdir(parents=True, exist_ok=True)
    if REPORT_COUNTER_FILE.exists():
        last = int(REPORT_COUNTER_FILE.read_text().strip())
        next_num = last + 1
    else:
        next_num = REPORT_COUNTER_START
    REPORT_COUNTER_FILE.write_text(str(next_num))
