...
def draw_cover(canvas, doc, meta, title_lines=None, subtitle=None):
    ...
    13.09.2026: title_lines/subtitle сделаны необязательными параметрами
    (по умолчанию — прежние константы COVER_TITLE_LINES/COVER_SUBTITLE,
    поведение для Чек-апа не меняется) — чтобы этот же builder переиспользовать
    для нового, более короткого PDF-отчёта Квиза с другим заголовком обложки."""
    canvas.saveState()
    page_w, page_h = A4
    title_lines = title_lines if title_lines is not None else COVER_TITLE_LINES
    subtitle = subtitle if subtitle is not None else COVER_SUBTITLE
    ...
    block_h = len(title_lines) * title_leading + 6 * mm + subtitle_leading
    ...
    canvas.drawCentredString(page_w / 2, y, subtitle)
...
def draw_footer(canvas, doc, meta, page_state=None):
    ...
    if meta.get("report_number"):
        footer_parts.append(f'Отчёт № {meta["report_number"]}')
    if meta.get("diagnosis_date"):
        footer_parts.append(f'Дата диагностики: {meta["diagnosis_date"]}')
    if meta.get("company"):
        footer_parts.append(meta["company"])
...
# ===========================================================================
# ОТЧЁТ ДЛЯ КВИЗА (П1, ветка КОНСУЛЬТАНТ) — 13.09.2026
# ---------------------------------------------------------------------------
# Отдельная, короткая функция для бесплатного триггер-продукта «Квиз»
# (НЕ путать с build_story() — полным платным Чек-ап-отчётом). Использует
# только общие визуальные примитивы (section_header, para, header_with_body,
# draw_cover/draw_footer с переопределённым заголовком обложки) — код самого
# build_story() не тронут и не переиспользован напрямую, во избежание риска
# для платного потока (см. обсуждение в чате 13.09.2026).
# ===========================================================================

QUIZ_COVER_TITLE_LINES = ["РАЗБОР РЕЗУЛЬТАТОВ", "ЭКСПРЕСС-ДИАГНОСТИКИ", "ПО СИСТЕМЕ", "«ВОЗРОЖДЕНИЕ БИЗНЕСА»"]
QUIZ_COVER_SUBTITLE = "Персональный отчёт"

_STAGE_NAMES = {
    1: "Стартап", 2: "Подъём", 3: "Делегирование", 4: "Профессиональная",
    5: "Интеграция", 6: "Стратегическая", 7: "Образ будущего",
}

QUIZ_STAGE_INTRO = (
    "«Стадии роста» — это методология, основанная на международных исследованиях более "
    "1500 компаний малого и среднего бизнеса в десятках отраслей. Цель исследования была "
    "простой: понять, почему одни компании растут устойчиво, а другие застревают или "
    "откатываются назад. Оказалось, что дело не в размере выручки, а в организационной "
    "сложности бизнеса — и эта сложность напрямую определяется количеством сотрудников, а не "
    "деньгами. Компания из 10 человек и компания из 200 — это 2 принципиально разных типа "
    "организации, с разными структурами, разными фокусами и разными правилами роста.\n\n"
    "Исследование выявило 7 таких этапов — Стадий роста. На каждой Стадии у компании свои "
    "типичные вызовы, свои точки роста и свои задачи, которые нужно решить, чтобы перейти на "
    "следующий уровень. На каждой Стадии есть конкретные правила, соблюдение которых создаёт "
    "условия для дальнейшего роста. Если правила не соблюдаются, бизнес не просто топчется на "
    "месте — он рискует откатиться назад.\n\n"
    "Знание своей Стадии — это не абстракция, а план действий. Это означает, что вы можете "
    "перестать изобретать решения с нуля и вместо этого понимать: то, с чем столкнулся ваш "
    "бизнес, уже проходили тысячи других компаний на этой же Стадии роста. И поэтому есть "
    "понятная и многократно проверенная логика, что и как делать дальше."
)

QUIZ_CHALLENGES_INTRO = (
    "Классические вызовы — это типичные, узнаваемые проблемы, с которыми компания сталкивается "
    "на своей Стадии роста: например, низкие продажи, кассовые разрывы, слабая бизнес-модель, "
    "нехватка подготовленных сотрудников. Исследования выявили 24 таких вызова — и на каждой "
    "Стадии актуален лишь определённый набор из них. Тех, что связаны именно с уровнем "
    "организационной сложности бизнеса.\n\n"
    "Дать вызову название — уже важно само по себе: с известным врагом бороться проще, чем с "
    "неопределённым ощущением, что «что-то не так». Общая терминология также упрощает "
    "взаимодействия внутри Управленческой команды — все понимают одни и те же проблемы "
    "одинаково и могут своевременно и сообща сфокусировать ресурсы на устранении вызовов.\n\n"
    "Но главное — Классические вызовы не существуют сами по себе. Это поверхностные симптомы "
    "более глубоких причин – т.н. «корневых причин». Система «Возрождение бизнеса» связывает "
    "каждый вызов с конкретными Ключевыми системными элементами, которые отсутствуют в "
    "экосистеме бизнеса. Компания, которая распознаёт эти вызовы вовремя, делает первый шаг к "
    "тому, чтобы понять, какой именно Системный элемент нужно внедрить — не для того, чтобы "
    "заглушить симптом, а чтобы устранить его истинную корневую причину.\n\n"
    "Вызовы, с которыми сталкивается ваш бизнес:"
)

QUIZ_NOT_COINCIDENCE = (
    "Может показаться, что список ваших вызовов — это просто набор разрозненных проблем — "
    "что-то с продажами, что-то с людьми, что-то с деньгами — и что каждую из таких проблем "
    "нужно решать отдельно, своим способом.\n\n"
    "Это не так. Вызовы не существуют по одному. Если вы отметили несколько — это не совпадение "
    "и не «просто трудный период». Это признак того, что экосистема вашего бизнеса вошла в "
    "дисбаланс и в ней не хватает одного или нескольких Системных элементов. Именно их "
    "отсутствие проявляется сразу в нескольких областях бизнеса одновременно — как волновой "
    "эффект, который появляется на воде от брошенного камня.\n\n"
    "Именно поэтому попытка устранить каждый вызов по отдельности (нанять ещё одного человека, "
    "внедрить ещё один инструмент) чаще всего даёт временное облегчение: симптом отступает на "
    "какое-то время и неизбежно возвращается. Потому что корневая причина вызова не устранена. "
    "Устойчивое решение лежит не в борьбе с вызовами, а в том, что стоит за ними и их порождает."
)

QUIZ_KSE_INTRO = (
    "Ключевые Системные элементы бизнеса — это принципы, методы и инструменты, которые "
    "образуют ключевые структуры в бизнесе, без которых система не может расти устойчиво: "
    "чёткая Бизнес-модель, Организационная структура, Коучинговое управление персоналом, "
    "Ценности бренда и другие — всего таких Системных элементов 11.\n\n"
    "Во многих организациях отсутствует какой-либо Системный элемент, или, по крайней мере, "
    "этот элемент недостаточно развит для поддержки текущей Стадии роста организации или "
    "дальнейшей фазы развития. В результате такой бизнес сталкивается с множеством проблем и "
    "их внешних проявлений.\n\n"
    "Разница между Классическими вызовами и Ключевыми системными элементами принципиальная. "
    "Вызов — это то, что болит, что вы ощущаете: низкие продажи, текучесть кадров, хаос в "
    "операционке. Ключевые системные элементы — это то, почему болит и чего в экосистеме "
    "бизнеса не хватает, из-за чего болевой синдром (т.е. вызов) вообще возникает. Именно "
    "поэтому Ключевые системные элементы — это не ещё один список проблем. Это то, что нужно "
    "внедрить в бизнесе, чтобы проблема ушла и перестала возвращаться.\n\n"
    "Здесь заканчивается диагностика и начинается лечение. Определив, каких именно Системных "
    "элементов не хватает вашему бизнесу, вы получаете не абстрактный совет «работайте лучше», "
    "а конкретный рабочий план: что именно нужно выстроить в экосистеме компании, чтобы "
    "устранить корневые причины, а не бороться по кругу с их симптомами.\n\n"
    "Ключевые системные элементы, которые необходимо внедрить в вашем бизнесе:"
)

QUIZ_CLOSING = (
    "То, что вы прошли — это экспресс-диагностика, первичный осмотр. Он крайне важен, но не "
    "заменяет полное обследование. Он показывает главное: что тревожный сигнал есть, и "
    "указывает направление, где искать причину.\n\n"
    "Вы находитесь на конкретной Стадии роста — не в вакууме, а на предсказуемом этапе, через "
    "который уже прошли тысячи компаний до вас. Вызовы, которые вы отметили, не случайны — за "
    "ними стоят конкретные Системные элементы, которых пока не хватает в экосистеме вашего "
    "бизнеса. Мы их для вас определили и указали в этом отчёте в порядке приоритета.\n\n"
    "Но этот диагноз предварительный, не окончательный. Первичный осмотр по своей природе не "
    "даёт всей картины — он показывает, где болит. Но он не может объяснить, что именно "
    "происходит на более глубоком уровне и что вызвало этот сбой.\n\n"
    "И здесь возникает ключевой вопрос — не «Что делать?», а «Как действовать при таком "
    "диагнозе?». Начинать ли работу с тем, что уже видно? Или сначала пройти полное "
    "обследование, чтобы увидеть всю картину целиком, прежде чем что-либо предпринимать? У "
    "обоих путей своя логика — и в следующем письме мы разберём, какой из них подойдёт именно "
    "вашей ситуации."
)


def build_quiz_explanation_story(lead_data: dict, data: dict) -> list:
    """Собирает содержимое (без обложки/футера — их рисуют draw_cover/
    draw_footer через doc.build) короткого персонального PDF-отчёта для
    клиентов Квиза, выбравших маршрут КОНСУЛЬТАНТ (письмо П1). Объясняет
    результаты экспресс-диагностики глубже, чем это возможно на экране
    ...
    # Вся вёрстка обложки рисуется абсолютными координатами в draw_cover()
    # (передаётся как onFirstPage в doc.build) — здесь только запускаем страницу,
    # без этого текст вступления рисовался бы поверх обложки (обнаружено 13.09.2026).
    story.append(Spacer(1, 1))
    story.append(PageBreak())

    # ---------- Вступление ----------
    story.append(para(f"Здравствуйте, {first_name}!", "H3"))
    story.append(Spacer(1, 3 * mm))
    story.append(para(
        "Вы прошли экспресс-диагностику вашего бизнеса по системе «Возрождение бизнеса». "
        "В этом Отчёте мы разберём для вас результаты диагностики.\n\n"
        "Здесь вы найдёте: на какой Стадии роста находится ваш бизнес и что это означает, "
        "разбор вызовов, которые вы указали, и то, какие Ключевые системные элементы стоят "
        "за этими вызовами — то есть не просто «что болит», а «почему болит»."
    ))
    story.append(PageBreak())

    # ---------- Раздел 1: Ваша Стадия ----------
    story.extend(section_header(1, "Ваша Стадия"))
    story.append(para(QUIZ_STAGE_INTRO))
    story.append(Spacer(1, 5 * mm))
    if stage_id:
        stage_name = _STAGE_NAMES.get(stage_id, "")
        stage_resume = data.get("stage_level_report_texts", {}).get(str(stage_id), {}).get("РЕЗЮМЕ", "")
        story.append(header_with_body(
            f"Вы находитесь на Стадии {stage_id} — {stage_name}",
            stage_resume or "Описание Стадии временно недоступно.",
        ))
    story.append(PageBreak())

    # ---------- Раздел 2: Подтверждённые вами вызовы ----------
    story.extend(section_header(2, "Подтверждённые вами вызовы"))
    story.append(para(QUIZ_CHALLENGES_INTRO))
    story.append(Spacer(1, 5 * mm))
    symptoms_data = data.get("challenge_symptoms_by_stage", {})
    for ch_name in challenge_names:
        symptoms = symptoms_data.get(ch_name, {}).get(str(stage_id), [])
        body = "\n".join(f"— {s}" for s in symptoms) if symptoms else "—"
        story.append(header_with_body(ch_name, body))
        story.append(Spacer(1, 3 * mm))
    story.append(PageBreak())

    # ---------- Раздел 3: Почему это не случайность ----------
    story.extend(section_header(3, "Почему это не случайность"))
    story.append(para(QUIZ_NOT_COINCIDENCE))
    story.append(PageBreak())

    # ---------- Раздел 4: Ваши приоритетные элементы ----------
    story.extend(section_header(4, "Ваши приоритетные Системные элементы"))
    story.append(para(QUIZ_KSE_INTRO))
    story.append(Spacer(1, 5 * mm))
    kse_data = data.get("kse_descriptions", {})
    for i, el_name in enumerate(top_elements, start=1):
        el_info = kse_data.get(el_name, {})
        story.append(header_with_body(f"{i}. {el_name}", el_info.get("описание", "—")))
        story.append(Spacer(1, 3 * mm))
    story.append(PageBreak())

    # ---------- Раздел 5: Что это значит для вас ----------
    story.extend(section_header(5, "Что это значит для вас"))
    story.append(para(QUIZ_CLOSING))

    return story


def generate_quiz_explanation_pdf(lead_data: dict) -> bytes:
    """Генерирует готовый PDF (bytes) для письма П1 — вызывается на лету
    маршрутом GET /quiz-explanation-pdf/<phone_e164> в app.py, ничего не
    сохраняя на диск. Использует ту же обложку/футер, что и Чек-ап
    (draw_cover/draw_footer), но с собственным заголовком обложки и без
    сквозного номера отчёта Чек-апа (report_number_seq не трогаем)."""
    import io

    data = sa.load_data()
    with open(BASE / "data" / "stage_level_report_texts.json", encoding="utf-8") as f:
        data["stage_level_report_texts"] = json.load(f)
    with open(BASE / "data" / "challenge_symptoms_by_stage.json", encoding="utf-8") as f:
        data["challenge_symptoms_by_stage"] = json.load(f)
    with open(BASE / "data" / "kse_descriptions.json", encoding="utf-8") as f:
        data["kse_descriptions"] = json.load(f)

    story = build_quiz_explanation_story(lead_data, data)

    buf = io.BytesIO()
    doc = SimpleDocTemplate(
        buf, pagesize=A4,
        leftMargin=18 * mm, rightMargin=18 * mm, topMargin=16 * mm, bottomMargin=16 * mm,
        title="Разбор результатов экспресс-диагностики",
    )
    meta = {
        "report_number": "",  # не используем сквозную нумерацию платных Чек-ап-отчётов
        "diagnosis_date": lead_data.get("submitted_at", "")[:10],
        "company": "",  # Квиз не собирает название компании
        "name": f'{lead_data.get("first_name", "")} {lead_data.get("last_name", "")}'.strip(),
    }
    doc.build(
        story,
        onFirstPage=partial(draw_cover, meta=meta, title_lines=QUIZ_COVER_TITLE_LINES, subtitle=QUIZ_COVER_SUBTITLE),
        onLaterPages=partial(draw_footer, meta=meta),
    )
    return buf.getvalue()
